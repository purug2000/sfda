...
class sfdaTrainer(Trainer):
        ...
        **kwargs,
        ...
        def _update_prototypes(self):
            self.prototypes, _ = APM_update(self.prediction_loop(self.get_train_dataloader(),
            description = F"APM Update @Global step {self.global_step}",ret_feats  = True), 
            flag = self.APM_Strategy,num_labels = self.num_labels, k = self.top_k,
            cf_ratio = self.cf_ratio )
        ...
        def compute_loss(self, model, inputs):
            ...
            # """
            prototypes = torch.Tensor(self.prototypes).to(self.args.device)
            outputs = model(**inputs,prototypes = prototypes, cf_ratio = self.cf_ratio , train_mode = "sfda")
            # Save past state if it exists
            if self.args.past_index >= 0:
                self._past = outputs[self.args.past_index]
            # We don't use .loss here since the model may return tuples instead of ModelOutput.
            s2t_loss = outputs.loss_s2t 
            return s2t_loss
            # t_loss = outputs.loss_t
